...
//
//  DisplayCharacteristicViewController.m
//  MyselfDemo
//
...
@interface DisplayCharacteristicViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation DisplayCharacteristicViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self createUI];
    //初始化数据
    _sect = [NSMutableArray arrayWithObjects:@"read value",@"write value",@"desc",@"properties", nil];
    _readValueArray = [[NSMutableArray alloc]init];
    _descriptors = [[NSMutableArray alloc] init];
    //配置ble委托
    [self babyDelegate];
    //读取服务
    _bluetooth.channel(channelOnCharacteristicView).characteristicDetails(self.currPeripheral,self.characteristic);
...
-(void)babyDelegate{
    ...
    [_bluetooth setBlockOnReadValueForCharacteristicAtChannel:channelOnCharacteristicView block:^(CBPeripheral *peripheral, CBCharacteristic *characteristics, NSError *error) {
    ...
    [_bluetooth setBlockOnDiscoverDescriptorsForCharacteristicAtChannel:channelOnCharacteristicView block:^(CBPeripheral *peripheral, CBCharacteristic *characteristic, NSError *error) {
    ...
    //设置读取Descriptor的委托
    [_bluetooth setBlockOnReadValueForDescriptorsAtChannel:channelOnCharacteristicView block:^(CBPeripheral *peripheral, CBDescriptor *descriptor, NSError *error) {
    ...
    //设置写数据成功的block
    [_bluetooth setBlockOnDidWriteValueForCharacteristicAtChannel:channelOnCharacteristicView block:^(CBCharacteristic *characteristic, NSError *error) {
        NSLog(@"setBlockOnDidWriteValueForCharacteristicAtChannel characteristic:%@ and new value:%@",characteristic.UUID, characteristic.value);
    }];
    
    //设置通知状态改变的block
    [_bluetooth setBlockOnDidUpdateNotificationStateForCharacteristicAtChannel:channelOnCharacteristicView block:^(CBCharacteristic *characteristic, NSError *error) {
...
-(void)setNotifiy:(id)sender{
    ...
    if (self.characteristic.properties & CBCharacteristicPropertyNotify ||  self.characteristic.properties & CBCharacteristicPropertyIndicate) {
        ...
        if(self.characteristic.isNotifying) {
            [_bluetooth cancelNotify:self.currPeripheral characteristic:self.characteristic];
            ...
            [_bluetooth notify:self.currPeripheral
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        case 0:
            //read value
        {
            cell.textLabel.text = [_readValueArray objectAtIndex:indexPath.row];
        ...
        case 2:
            //desc
        {
            CBDescriptor *descriptor = [_descriptors objectAtIndex:indexPath.row];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    switch (section) {
            ...
            //write value
        {
            ...
            title.text = [_sect objectAtIndex:section];
            ...
            //恢复状态
            if(self.characteristic.isNotifying) {
                [_bluetooth notify:self.currPeripheral characteristic:self.characteristic block:^(CBPeripheral *peripheral, CBCharacteristic *characteristics, NSError *error) {

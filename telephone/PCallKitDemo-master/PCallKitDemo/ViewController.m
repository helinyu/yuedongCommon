...
- (void)viewDidLoad {
    ...
    noTextField = [[UITextField alloc] initWithFrame:CGRectMake((self.view.frame.size.width-100)/2, 50, 200, 20)];
...
-(void)checkPermissions:(UIButton *)sender
{
    ...
    // 获取权限状态
    [manager getEnabledStatusForExtensionWithIdentifier:@"com.sport.yuedong.YDTodayExtension" completionHandler:^(CXCallDirectoryEnabledStatus enabledStatus, NSError * _Nullable error) {
        if (!error) {
            NSString *title = nil;
...
{
    CXCallDirectoryManager *manager = [CXCallDirectoryManager sharedInstance];
    [manager reloadExtensionWithIdentifier:@"com.sport.yuedong.YDTodayExtension" completionHandler:^(NSError * _Nullable error) {
        if (error == nil) {
            UIAlertController* alert = [UIAlertController alertControllerWithTitle:@"提示"
                                                                           message:@"更新成功"
                                                                    preferredStyle:UIAlertControllerStyleAlert];

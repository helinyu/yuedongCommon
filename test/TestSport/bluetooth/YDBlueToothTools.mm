...
- (void)scanClick
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"yd.reload" object:nil userInfo:nil];
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    for (int i=0; i < _nDevices.count; i++) {
        ...
        if ([p isEqual:peripheral]) {
            ...
            replace = YES;
            [[NSNotificationCenter defaultCenter] postNotificationName:@"yd.reload" object:nil];
//            [self.manager stopScan];
        }
    }
    if (!replace) {
...
//        NSLog(@"c.UUID = %@",c.UUID);//FFF2 发数据
        ...
        if ([c.UUID isEqual:[CBUUID UUIDWithString:@"FFF1"]]) {
            Byte data1[] = {0x01};
            NSData* writeDatas1 = [[NSData alloc] initWithBytes:data1 length:1];
            [_peripheral writeValue:writeDatas1 forCharacteristic:c type:CBCharacteristicWriteWithResponse];
            
            Byte data2[] = {0x02};
            NSData* writeDatas2 = [[NSData alloc] initWithBytes:data2 length:1];
            [_peripheral writeValue:writeDatas2 forCharacteristic:c type:CBCharacteristicWriteWithResponse];

            Byte data3[] = {0x03};
            NSData* writeDatas3 = [[NSData alloc] initWithBytes:data3 length:1];
            [_peripheral writeValue:writeDatas3 forCharacteristic:c type:CBCharacteristicWriteWithResponse];
            
            Byte data4[] = {0x04};
            NSData* writeDatas4 = [[NSData alloc] initWithBytes:data4 length:1];
            [_peripheral writeValue:writeDatas4 forCharacteristic:c type:CBCharacteristicWriteWithResponse];
          
            Byte data5[] = {0x05};
            NSData* writeDatas5 = [[NSData alloc] initWithBytes:data5 length:1];
            [_peripheral writeValue:writeDatas5 forCharacteristic:c type:CBCharacteristicWriteWithResponse];
            
            Byte data6[] = {0x06};
            NSData* writeDatas6 = [[NSData alloc] initWithBytes:data6 length:1];
            [_peripheral writeValue:writeDatas6 forCharacteristic:c type:CBCharacteristicWriteWithResponse];
...
//        NSLog(@"index:%d ,vlaue: %hhu",i,resultByte[i]);
    }
...
//        NSLog(@"接收数据校验失败");
//    }
    Byte playload[resultByte[1] - 1];
    for (int i = 0; i < resultByte[1] - 1; i++) {
        playload[i] = resultByte[2 + i];
    }
    return readProtocolDataBytes(playload,resultByte[1] - 1);
}

...
-(void)DidUpdataValueForCharacteristic:(NSNotification *)notification{
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"FFF2"]]) {
        ...
        Byte * resultByte = (Byte *)[data bytes]; // 数据的协议进行处理
            ...
            if (i == 2) {
                ...
                if (resultByte[1] == 9) {
                    ...
                    if ([UIDevice currentDevice].systemVersion.floatValue >= 8.f) {
                        ...
                        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"运动模式未开启" message:@"(请在蓝牙耳机开启运动模式按钮)" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                        [alert show];
                    }
...
- (S3MainView *)mainView{
    if (!_mainView) {
        ...
        _mainView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT -64);
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex NS_DEPRECATED_IOS(2_0, 9_0) {
    NSLog(@"clicke index ; %d",buttonIndex);
}

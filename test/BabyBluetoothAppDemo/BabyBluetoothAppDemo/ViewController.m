...
//    状态发生改变
    ...
    //设置扫描到设备的委托 ，扫描到了设备，发现有了什么设备都会返回来的
    [baby setBlockOnDiscoverToPeripherals:^(CBCentralManager *central, CBPeripheral *peripheral, NSDictionary *advertisementData, NSNumber *RSSI) {
        NSLog(@"搜索到了设备:%@",peripheral.name);
        [weakSelf insertTableView:peripheral advertisementData:advertisementData RSSI:RSSI];
    }];
    
#warning  发现对应的charcteric
    //设置发现设service的Characteristics的委托， 发现对应的设备的服务特征  == 》 看到block的参数，应该是找peripheral对应的服务
    ...
    //设置读取characteristics的委托，读取设备服务是上的特征的时候，就会返回这个参数， 找到了服务对应的特征  ,
    //    读取对应的charactistic ，对应的信息
    [baby setBlockOnReadValueForCharacteristic:^(CBPeripheral *peripheral, CBCharacteristic *characteristics, NSError *error) {
        NSLog(@"characteristic name:%@ value is:%@",characteristics.UUID,characteristics.value);
    }];
    
#warning descriptor 对应的内容
    //设置发现characteristics的descriptors的委托  desciptors 描述 和特征是两个不同的概念的相关信息了

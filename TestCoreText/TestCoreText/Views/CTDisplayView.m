...
- (void)setupEvents {
    ...
    UIGestureRecognizer *panRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(userPanGuestureDetected:)];
...
- (void)userTapGestureDetected:(UIGestureRecognizer *)recognizer {
    ...
    if (_state == CTDisplayViewStateNormal) {
        for (YDCTImageModel * imageData in self.data.imgs) {
        ...
        YDCTLinkModel *linkData = [YDCTUtils touchLinkInView:self atPoint:point data:self.data];
        if (linkData) {
            NSLog(@"hint link!");
...
- (void)userLongPressedGuestureDetected:(UILongPressGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan ||
        ...
        CFIndex index = [YDCTUtils touchContentOffsetInView:self atPoint:point data:self.data];
